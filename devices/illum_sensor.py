...
def grpc_get_illumination(
    stub: sv_device_grpc.DeviceServiceStub, map_id: MapId
) -> sv_device.ResponseSensorIllumination:
    response: sv_device.ResponseSensorIllumination = stub.GetIllumination(
...
def grpc_handle_response(response: sv_device.ResponseSensorIllumination) -> bool:
...
if __name__ == "__main__":
    ...
    while True:
        try:
            response = grpc_get_illumination(grpc_stub, map_id)
            if grpc_handle_response(response):
                payload = json.dumps({"illumimation": response.illumination.value})
