...
def grpc_get_temperature(
    stub: sv_device_grpc.DeviceServiceStub, map_id: MapId
) -> sv_device.ResponseSensorAirTemperature:
    response: sv_device.ResponseSensorAirTemperature = stub.GetAirTemperature(
...
def grpc_handle_response(response: sv_device.ResponseSensorAirTemperature) -> bool:
...
if __name__ == "__main__":
    ...
    while True:
        try:
            response = grpc_get_temperature(grpc_stub, map_id)
            if grpc_handle_response(response):
                payload = json.dumps({"temp": response.temp.value})
